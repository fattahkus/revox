    ...
    try{
        ...
        for (let index = 0; index < pkData.length; index++){
            ...
            if(pkData[index]){
                ...
                const value = `0.00000025${randvalue}`
                const amountToSend = web3.utils.toWei(value, 'ether'); // Amount in BNB
                ...
                if(await getBalance(mainWallet.address) > 0.0000005){
                    ...
                    if(getBalances < 0.00000025 ){
                        ...
                        await web3.eth.accounts.signTransaction(transaction, MainPrivateKey)
                          ...
                          .then(receipt => {
                            console.log(`[${indexs}/${tot}] Send ${receipt.from} : ${value} to ${receipt.to} : ${getBalances} | Tx hash : ${receipt.transactionHash}`); 
                          })
                          .catch(error => {
